      ...
      {messages &&
        messages.map((m, i) => (
          <div style={{ display: "flex" }} key={m._id}>
            ...
            >{m.content ? (
              m.content.includes("[PRDCT]") ? (
                (() => {
                  const productId = m.content.split("[PRDCT]")[1];
                  console.log(productId);
                  const product = womenProducts.find(pord => `(${pord.id})` == productId);
                  console.log(product);
                  return product ? (
                    <div className='col-span-1 p-4 border rounded-lg'>
                      ...
                      <p className='font-semibold'>
                        {product.price} 
                        <span className='line-through text-gray-400'>{product.strikedoffprice}</span> 
                        <span className='text-red-500'>{product.offer}</span>
                      </p>
                      <button className='mt-2 mr-4 p-2 bg-pink-500 text-white rounded'>View Product</button>
                    </div>
                  ) : (
              ...
              <img src={m.media} alt="media" style={{ width: "100%" }} />
            )}
